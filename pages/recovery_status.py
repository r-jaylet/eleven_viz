    ...
    try:
        ...
        tabs = st.tabs(
            [
                ...
                "Match & Recovery (pre-match & post-match)",
                "Category Breakdown",
                "Weekly Patterns",
            ]
        )
        ...
        with tabs[1]:
            ...
            st.subheader(
                "Comparison of pre- vs post-match recovery scores for each match using a dumbbell chart"
            )
            ...
            fig_pre_post_match_recovery = (
                plot_pre_post_match_recovery_dumbbell(
                    df_gps, df, recovery_metric="soreness_baseline_composite"
                )
            )
            st.plotly_chart(
                fig_pre_post_match_recovery, use_container_width=True
            )
            ...
            st.markdown(
                "This graph shows a readiness score for each match, calculated as the average of the player’s benchmark performance score (latest benchmarkPct) and recovery score (from 1–2 days before the match). "
        ...
        with tabs[2]:
            ...
            with col2:
                ...
                st.markdown(
                    """This graph displays composite recovery scores across multiple wellness categories (e.g. soreness, sleep, subjective, bio) over time. """
                )
